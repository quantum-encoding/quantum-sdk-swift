...
public final class QuantumClient: Sendable {
    ...
    /// Generic SSE stream builder for agent/mission endpoints. The body is
    /// encoded before the stream's task starts, so the `Encodable` itself
    ...
    func makeSSEStream<T>(
        ...
        let encoded = Result { try JSONEncoder().encode(EncodableWrapper(body)) }
        return AsyncThrowingStream { continuation in
            let task = Task {
                do {
                    let bodyData = try encoded.get()
                    let (bytes, _) = try await self.http.doStreamRequest(
                        method: "POST", path: path, bodyData: bodyData, idempotencyKey: nil
                    )
